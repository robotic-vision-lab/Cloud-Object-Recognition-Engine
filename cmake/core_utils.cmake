...
macro(DISSECT_VERSION)
  ...
  string(REGEX REPLACE "^([0-9]+).*" "\\1"
         CORE_MAJOR_VERSION "${CORE_VERSION}")
  string(REGEX REPLACE "^[0-9]+\\.([0-9]+).*" "\\1"
         CORE_MINOR_VERSION "${CORE_VERSION}")
  string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.([0-9]+)" "\\1"
         CORE_REVISION_VERSION "${CORE_VERSION}")
  string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.[0-9]+(.*)" "\\1"
         CORE_CANDIDATE_VERSION "${CORE_VERSION}")
...
# such as WIN32, APPLE, and CYGWIN.
...
# subdirectory named after the library in question.
macro(SET_INSTALL_DIRS)
  if(NOT DEFINED LIB_INSTALL_DIR)
    set(LIB_INSTALL_DIR "lib")
  endif (NOT DEFINED LIB_INSTALL_DIR)
    set(INCLUDE_INSTALL_ROOT
        "include/${PROJECT_NAME_LOWER}-${CORE_MAJOR_VERSION}.${CORE_MINOR_VERSION}")
    set(INCLUDE_INSTALL_DIR "${INCLUDE_INSTALL_ROOT}/core")
    set(DOC_INSTALL_DIR "share/doc/${PROJECT_NAME_LOWER}-${CORE_MAJOR_VERSION}.${CORE_MINOR_VERSION}")
    set(BIN_INSTALL_DIR "bin")
    set(PKGCFG_INSTALL_DIR "${LIB_INSTALL_DIR}/pkgconfig")
    if(WIN32)
      set(CORECONFIG_INSTALL_DIR "cmake")
    else(WIN32)
      set(CORECONFIG_INSTALL_DIR "share/${PROJECT_NAME_LOWER}-${CORE_MAJOR_VERSION}.${CORE_MINOR_VERSION}")
    endif(WIN32)
...
##########################################################################
# This function was copied from boost-cmake project.                     #
# The license terms is as follows:                                       #
##########################################################################
      ...
      while(STACK_LENGTH GREATER 0)
        ...
        while(OUT_DEGREE GREATER 0)
          ...
          if(NOT FOUND_${UPPER_TARGET})
            ...
            set(OUT_EDGES ${${PREFIX}${UPPER_SOURCE}${SUFFIX}})
...
# _list [IN/OUT] A list.
# _pos1 [IN] The position of the first element.
# _pos2 [IN] The position of the second element.
...
macro(swap_elements _list _pos1 _pos2)
  ...
  unset(element1)
  unset(element2)
  # Sort pos1 and pos2 such that pos1 < pos2
...
# _position The position of the element to set.
# _value The new element value.
...
macro(sort_relative _list _sorted_list _to_sort_relative)
  ...
  # Ensure sizes are equal for the three lists, otherwise fail gracefully
  ...
  if(NOT (list_length EQUAL sorted_list_length))
    message(FATAL_ERROR "size mismatch between ${_sorted_list} (length 
            ${sorted_list_length}) and ${_list} (length ${list_length})")
  endif(NOT (list_length EQUAL sorted_list_length))

  if(NOT (list_length EQUAL to_sort_list_length))
    message(FATAL_ERROR "size mismatch between ${_to_sort_relative} 
            ${to_sort_list_length} and ${_list} ${list_length}")
  endif(NOT (list_length EQUAL to_sort_list_length))
  # Unset the temporary list to avoid surprises 
  unset(tmp_list)
